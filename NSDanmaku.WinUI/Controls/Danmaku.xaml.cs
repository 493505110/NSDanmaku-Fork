...
namespace NSDanmaku.WinUI.Controls
{
    public sealed partial class Danmaku : UserControl
    {
        ...
        private void SetFontDanmakuSizeZoom(double value)
        {
            ...
            foreach (var item in scrollContainer.Children)
            {
            ...
            foreach (var item in bottomContainer.Children)
            {
                var grid = item as Grid;
                var m = grid.Tag as DanmakuModel;
                foreach (var tb in grid.Children)
        ...
        private void SetRows(double height)
        {
            ...
            //将全部行去除
            topContainer.RowDefinitions.Clear();
            bottomContainer.RowDefinitions.Clear();
            scrollContainer.RowDefinitions.Clear();
            ...
            //for (int i = 0; i < num; i++)
            //{
            //    bottomContainer.RowDefinitions.Add(new RowDefinition());
            //    topContainer.RowDefinitions.Add(new RowDefinition());
            //}
            for (int i = 0; i < num; i++)
            {
                bottomContainer.RowDefinitions.Add(new RowDefinition());
                topContainer.RowDefinitions.Add(new RowDefinition());
                scrollContainer.RowDefinitions.Add(new RowDefinition());
            }
        }
        private int GetTopAvailableRow()
        {

            var max = topContainer.RowDefinitions.Count / 2;

            for (int i = 0; i < max; i++)
            {

                var row = topContainer.Children.FirstOrDefault(x => Grid.GetRow((x as Grid)) == i);
                if (row != null)
                {
                    continue;
                }
        ...
        private int GetBottomAvailableRow()
        {
            ...
            var max = bottomContainer.RowDefinitions.Count / 2;
            for (int i = 1; i <= max; i++)
            {
                var rowNum = bottomContainer.RowDefinitions.Count - i;
                var row = bottomContainer.Children.FirstOrDefault(x => Grid.GetRow((x as Grid)) == rowNum);
                if (row != null)
                {
                    continue;
                }
            ...
            //for (int i = bottomContainer.RowDefinitions.Count; i >= 0; i--)
            //{
            //    var row = bottomContainer.Children.FirstOrDefault(x => Grid.GetRow((x as Grid)) == i);
            //    if (row != null)
            //    {
            //        continue;
            //    }
        ...
        private int GetScrollAvailableRow(Grid item)
        {
            ...
            var max = scrollContainer.RowDefinitions.Count * DanmakuArea;

            for (int i = 0; i < max; i++)
            {
                //1、检查当前行是否存在弹幕
                var lastItem = scrollContainer.Children.LastOrDefault(x => Grid.GetRow((x as Grid)) == i);
                if (lastItem == null)
                {
                    return i;
                }
        ...
        public async void AddLiveDanmu(string text, bool own, Color? color)
        {
            ...
            scrollContainer.Children.Add(grid);
            ...
            {
                scrollContainer.Children.Remove(grid);
                grid.Children.Clear();
                grid = null;
        ...
        public async Task AddTopDanmu(DanmakuModel m, bool own)
        {
            ...
            Grid.SetRow(grid, r);
            topContainer.Children.Add(grid);
            ...
            moveStoryboard.Completed += new EventHandler<object>((senders, obj) =>
            {
                topContainer.Children.Remove(grid);
                grid.Children.Clear();
                grid = null;
        ...
        public async Task AddBottomDanmu(DanmakuModel m, bool own)
        {
            ...
            bottomContainer.Children.Add(grid);
            ...
            moveStoryboard.Completed += new EventHandler<object>((senders, obj) =>
            {
                bottomContainer.Children.Remove(grid);
                grid.Children.Clear();
                grid = null;
                topBottomStoryList.Remove(moveStoryboard);
                moveStoryboard.Stop();
        ...
        public async Task AddPositionDanmu(DanmakuModel m)
        {
            ...
            danmakuCanvas.Children.Add(grid);
            ...
            moveStoryboard.Completed += new EventHandler<object>((senders, obj) =>
            {
                danmakuCanvas.Children.Remove(grid);
        ...
        public void Remove(DanmakuModel danmaku)
        {
            switch (danmaku.location)
            {
                case DanmakuLocation.Top:
                    ...
                    foreach (Grid item in topContainer.Children)
                    {
                        if (item.Tag as DanmakuModel == danmaku)
                        {
                            topContainer.Children.Remove(item);
                        }
                    }
                    break;
                case DanmakuLocation.Bottom:
                    foreach (Grid item in bottomContainer.Children)
                    {
                        if (item.Tag as DanmakuModel == danmaku)
                        {
                            bottomContainer.Children.Remove(item);
                        }
                    }
                    break;
                case DanmakuLocation.Other:
                    foreach (Grid item in scrollContainer.Children)
                    {
                        if (item.Tag as DanmakuModel == danmaku)
                        {
                            scrollContainer.Children.Remove(item);
                        }
                    }
                    break;
                default:
        ...
        public void ClearAll()
        {
            ...
            scrollContainer.Children.Clear();
        ...
        public void ShowDanmaku(DanmakuLocation location)
        {
            switch (location)
            {
                ...
                case DanmakuLocation.Top:
                    ...
                    break;
                case DanmakuLocation.Bottom:
                    bottomContainer.Visibility = Visibility.Visible;
                    break;
                default:
                    break;
            }
